...
signal enlever_crayon_inventaire
signal debut_tests
signal explication_finie
...
func _input(event):
	if state_finished and event is InputEventMouseButton and event.is_pressed() and not event.is_echo():
		...
		if state == 0 :
			...
			state_finished = false
			state += 1
			$armee_ennemie.montrer()
			lance_dialogue.emit() #oh non on nous attaque !!
			await get_tree().create_timer(5).timeout
			explication_finie.emit()
			state_finished = true
...
func _on_inv_papier_crayon_pressed():
	print("papier dej ouvert : ", papier_tests_ouvert)
	if not papier_tests_ouvert :
		papier_tests_ouvert = true
		$papier_tests.sortir()


func _on_bouton_deplier_pressed(): #state = 8
	state += 1
	$messager.scytale_vide()
	$bouton_compris.show()
...
func _on_bouton_compris_pressed(): #state = 9
	state_finished = false
	state += 1
	$bouton_compris.hide()
	$messager.sortirScytale()
	$papier_tests.hide()
	enlever_crayon_inventaire.emit()
	lance_dialogue.emit()
	state_finished = true
